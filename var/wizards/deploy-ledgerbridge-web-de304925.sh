...
core_state() {
  local probe_tree=$1 revision migration payroll_read_ready
  ...
  printf '%s|%s|%s\n' "$revision" "$migration" "$payroll_read_ready"
}
...
if [[ -n "$EXPECTED_CORE_REVISION" \
      ...
      && "$ACTIVE_PAYROLL_READ_READY" == 1 ]]; then
  CORE_READY=1
fi

if [[ "$MODE" == plan ]]; then
  printf 'WEB_RELEASE_PLAN_OK revision=%s archive_sha256=%s core_ready=%s payroll_read_ready=%s core_migration=%s target=web-only\n' \
    "$EXPECTED_REVISION" "$EXPECTED_ARCHIVE_SHA256" "$CORE_READY" \
    "$ACTIVE_PAYROLL_READ_READY" "$ACTIVE_CORE_MIGRATION"
  exit 0
fi
