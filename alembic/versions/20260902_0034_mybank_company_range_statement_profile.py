"""Admit the official MYbank company range-statement parser profile.
...
DECLARE
    ...
    v_profile_replacement text := $replacement$    ELSIF (p_request->>'institution_code') = 'mybank'
       AND (p_request->>'source_system') = 'mybank_company_statement'
       AND (p_request->>'declared_media_type') =
           'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
       AND v_profile = 'mybank_company_range_xlsx_v3' THEN
        NULL;
    ELSIF (p_request->>'institution_code') = 'ccb'
       AND (p_request->>'source_system') = 'ccb_personal_xls_export'$replacement$;
    v_owner_anchor text := $anchor$           v_profile = 'mybank_company_daily_xlsx_v2'
...
BEGIN
    ...
    IF strpos(v_definition, v_profile_anchor) = 0
       ...
       OR strpos(v_definition, 'mybank_company_range_xlsx_v3') <> 0 THEN
        RAISE EXCEPTION 'bank statement import function baseline changed';
    END IF;
    v_definition := replace(
        v_definition,
        v_profile_anchor,
        v_profile_replacement
    );
    v_definition := replace(
        v_definition,
        v_owner_anchor,
