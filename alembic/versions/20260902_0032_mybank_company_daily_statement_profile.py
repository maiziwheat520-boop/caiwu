"""Admit the MYbank company daily profile through the existing statement seam.
...
down_revision: str | None = "20260902_0031"
...
BEGIN
    ...
    ELSIF (p_request->>'institution_code') = 'mybank'
       AND (p_request->>'source_system') = 'mybank_daily_statement'
       AND (p_request->>'declared_media_type') =
           'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
       AND v_profile = 'mybank_company_daily_xlsx_v2' THEN
        NULL;
    ELSIF (p_request->>'institution_code') = 'ccb'
       ...
       AND v_profile = 'ccb_personal_xls_v1' THEN
        NULL;
    ELSIF (p_request->>'institution_code') = 'boc'
       AND (p_request->>'source_system') = 'boc_transaction_statement'
       AND (p_request->>'declared_media_type') = 'application/pdf'
       AND v_profile = 'boc_personal_pdf_v1' THEN
        NULL;
    ELSIF (p_request->>'institution_code') = 'abc'
       AND (p_request->>'source_system') = 'abc_personal_pdf_export'
       AND (p_request->>'declared_media_type') = 'application/pdf'
       AND v_profile = 'abc_personal_pdf_v1' THEN
        NULL;
    ELSE
    ...
    IF v_account_institution IS DISTINCT FROM p_request->>'institution_code'
       ...
       OR (
           ...
           AND v_account_owner_kind <> 'PERSONAL'
       )
       OR (
           v_profile = 'mybank_company_daily_xlsx_v2'
           AND v_account_owner_kind <> 'COMPANY'
       ) THEN
