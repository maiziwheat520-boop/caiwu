...
8. 其他电脑或手机要独立登录时，先在已登录页面的账户菜单选择“添加这台设备”。系统先要求使用任意一把已有 Passkey 做二次确认，再为当前设备创建新 Passkey。该正常追加流程不会撤销已有设备、恢复码或会话；每个账户最多登记 10 个。不要用恢复流程代替日常的设备登记，因为恢复流程按设计会撤销旧认证材料。
