...
WITH expected_roles(role_name) AS (
    ...
    SELECT 'public'::text, object_name, 'table'::text
      FROM unnest(ARRAY[{_R1_PUBLIC_TABLE_SQL}]::text[]) AS object(object_name)
    UNION ALL
    SELECT 'internal_read'::text, object_name, 'view'::text
      FROM unnest(ARRAY[{_R1_VIEW_SQL}]::text[]) AS object(object_name)
    UNION ALL
    ...
    SELECT COALESCE(json_agg(json_build_object(
     ...
     WHERE (n.nspname = 'public' AND c.relname IN ({_R1_PUBLIC_TABLE_SQL}))
    ...
    SELECT COALESCE(json_agg(json_build_object(
     ...
     WHERE NOT t.tgisinternal
       AND ((n.nspname = 'public' AND c.relname IN ({_R1_PUBLIC_TABLE_SQL}))
         OR (n.nspname = 'internal_read' AND c.relname = 'evidence_read_receipt'))
), r1_views AS (
    SELECT COALESCE(json_agg(json_build_object(
        'schema', n.nspname,
    ...
    SELECT COALESCE(json_agg(json_build_object(
      ...
      FROM pg_proc AS p
      JOIN pg_namespace AS n ON n.oid = p.pronamespace
     WHERE (n.nspname = 'internal_read' AND p.proname IN ({_R1_FUNCTION_SQL}))
